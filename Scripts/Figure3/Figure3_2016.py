...
MCWDFiles = []
data2016 = np.zeros((len(setup.files), 1946))
...
for countFile in range(0, 3):
    ...
    axGeo.text(-81, 5, r'$'+ lowerletters[index - 1+6] +'$)',
...
cb2.ax.set_xticklabels(['1 (None)', '2', '3', '4', '5', '6 (All)'])

plt.subplots_adjust(top = 0.8, bottom=  0.25, wspace=0.18, left = 0.15)

#bar = plt.colorbar.ColorbarBase(cax = cax, cmap=cmap, norm=norm, spacing='proportional', format='%1i')
plt.savefig("Drought-MCWD-Agreement-16-lessDatasets.png",  dpi=600, bbox_inches = 'tight',
    pad_inches = 0.0)
#plt.show()