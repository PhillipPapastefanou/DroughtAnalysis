...
MCWDFiles = []
data2005 = np.zeros((len(files), 1946))
data2010 = np.zeros((len(files), 1946))
...
for MCWDFile in MCWDFiles:
    data = MCWDFile.ParseRelativeDeviation(2001, 2010)
    data2005[i] = data[2005-2001]
    data2010[i] = data[2010-2001]
    i += 1
...
for d in range(0, 6):
    for j in range(0, len(files)):
        ...
        subdf = pandas.DataFrame({"Year": year,
                                  'Dataset': files[j][0],
                                  'Condition': d % 3,
                                  'TotalArea': acc[j,d],
...
df.to_csv(r'2005-2010Agreement.tsv', sep= '\t', header = True)
