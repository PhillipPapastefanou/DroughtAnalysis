...
for file in files:
    path = setup.MCWDrootPAth + "\\" + file[1]
    geoFile = OMAGeoFile(path)
    yearFrom = geoFile.year_begin
    yearTo = geoFile.year_end
    data = geoFile.GetAllGridcellsYearSpan("mcwd", 2001, 2016)
...
plt.savefig("Empirical_CDF_MCWD.png", dpi = 300)


import csv

with open("outputCDF_MCWD.csv", "w", newline="") as f:
    writer = csv.writer(f)
