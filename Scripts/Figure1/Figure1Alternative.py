...
cmapPrec = mpl.colors.ListedColormap(['#e6194B',
                                    '#f58231',
                                      '#ffe119',
                                      'white',
                                      '#bfef45',
...
for i in range(0, len(scenNames)):
    ...
    axGeo.text(0.97, 0.93, scenNames[index-1], horizontalalignment='right', verticalalignment='center',
            transform=axGeo.transAxes, size=12, bbox=dict(facecolor='white', alpha=0.8, edgecolor='white'))
...
patches.append(mpatches.Patch(label='x < -3', facecolor= uColor, edgecolor='black'))
patches.append(mpatches.Patch(label='-3 < x < -2', facecolor=cmapPrec.colors[0], edgecolor='black'))
patches.append(mpatches.Patch(label='-2 < x < -1', facecolor=cmapPrec.colors[1], edgecolor='black'))
patches.append(mpatches.Patch(label='-1 < x < -0.5', facecolor=cmapPrec.colors[2], edgecolor='black'))
patches.append(mpatches.Patch(label='-0.5 < x < 0.5', facecolor=cmapPrec.colors[3], edgecolor='black'))
patches.append(mpatches.Patch(label='0.5 < x < 1', facecolor=cmapPrec.colors[4], edgecolor='black'))
patches.append(mpatches.Patch(label='1 < x < 2', facecolor=cmapPrec.colors[5], edgecolor='black'))
patches.append(mpatches.Patch(label='2 < x < 3', facecolor=cmapPrec.colors[6], edgecolor='black'))
patches.append(mpatches.Patch(label='x > 3', facecolor=oColor, edgecolor='black'))

fig.tight_layout()
legend = fig.legend(handles=patches[0:4], loc='lower left', prop={"size": 14}, title="$\Longleftarrow$ More drought stress", frameon=False, ncol=2,
                    bbox_to_anchor=(0.1, -0.15))

legend.get_title().set_fontsize('14') #legend 'Title' fontsize

legend = fig.legend(handles=[patches[4]], loc='lower center', prop={"size": 14}, title="Relative MCWD anomaly\n            (aMCWD)", frameon=False, ncol=2,
...
legend = fig.legend(handles=patches[5:], loc='lower right', prop={"size": 14}, title="Less drought stress $\Longrightarrow$", frameon=False, ncol=2,
